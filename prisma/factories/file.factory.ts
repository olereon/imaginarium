...
export class FileFactory extends BaseFactory<FileUpload> {
  ...
  build(overrides: Partial<FileCreateInput> = {}): FileUpload {
    ...
    const storageProvider =
      overrides.storageProvider || this.randomElement(['local', 's3', 'gcs', 'azure']);
    ...
    const defaultData: FileCreateInput = {
      ...
      originalName:
        overrides.originalName || `original_${fileType.name}_${sequence}.${fileType.extension}`,
      ...
      s3Key:
        overrides.s3Key ||
        (storageProvider === 's3'
          ? `files/${sequence}/${fileType.name}.${fileType.extension}`
          : null),
      ...
      dimensions:
        ...
        (fileType.category === 'image'
          ? JSON.stringify({ width: this.randomInt(100, 4000), height: this.randomInt(100, 4000) })
          : null),
      duration:
        overrides.duration ||
        (fileType.category === 'video' || fileType.category === 'audio'
          ? this.randomInt(1, 3600)
          : null),
      quality:
        overrides.quality ||
        (fileType.category === 'image' || fileType.category === 'video'
          ? this.randomElement(['low', 'medium', 'high', 'ultra'])
          : null),
      compression:
        overrides.compression ||
        (fileType.category === 'image' ? this.randomElement(['none', 'lossless', 'lossy']) : null),
      status:
        overrides.status ||
        this.randomElement<FileStatus>(['PENDING', 'PROCESSING', 'READY', 'FAILED']),
      processingError:
        overrides.processingError ||
        (this.randomBoolean(0.1) ? 'Processing failed due to invalid format' : null),
      processingStage:
        overrides.processingStage ||
        (this.randomBoolean(0.3)
          ? this.randomElement(['upload', 'scan', 'process', 'optimize'])
          : null),
      ...
      encryptionKey:
        overrides.encryptionKey || (this.randomBoolean(0.3) ? this.generateEncryptionKey() : null),
      isEncrypted:
        overrides.isEncrypted !== undefined ? overrides.isEncrypted : this.randomBoolean(0.3),
      expiresAt: overrides.expiresAt || (this.randomBoolean(0.4) ? this.randomDate(-30) : null), // 30 days from now
      lastAccessedAt:
        overrides.lastAccessedAt || (this.randomBoolean(0.8) ? this.randomDate(7) : null),
      downloadCount: overrides.downloadCount || this.randomInt(0, 1000),
      virusScanned:
        overrides.virusScanned !== undefined ? overrides.virusScanned : this.randomBoolean(0.9),
      virusScanResult:
        overrides.virusScanResult || (this.randomBoolean(0.95) ? 'clean' : 'infected'),
  ...
  async createImageFile(overrides: Partial<FileCreateInput> = {}): Promise<FileUpload> {
    ...
    return await this.create({
      ...
      dimensions: JSON.stringify({
        width: this.randomInt(800, 4000),
        height: this.randomInt(600, 3000),
      }),
  ...
  async createVideoFile(overrides: Partial<FileCreateInput> = {}): Promise<FileUpload> {
    ...
    return await this.create({
      ...
      dimensions: JSON.stringify({
        width: this.randomInt(1280, 3840),
        height: this.randomInt(720, 2160),
      }),
  ...
  async createArtifact(
    runId: string,
    overrides: Partial<ArtifactCreateInput> = {}
  ): Promise<Artifact> {
    const sequence = this.getSequence('artifact');
    const artifactType =
      overrides.type ||
      this.randomElement<ArtifactType>([
        'IMAGE',
        'VIDEO',
        'AUDIO',
        'TEXT',
    ...
    const artifactData: ArtifactCreateInput = {
      runId,
      taskId:
        overrides.taskId || (this.randomBoolean(0.7) ? `task_${this.randomInt(1, 10)}` : null),
      fileId:
        overrides.fileId || (this.randomBoolean(0.5) ? `file_${this.randomInt(1, 100)}` : null),
      type: artifactType,
      ...
      parentId:
        overrides.parentId ||
        (this.randomBoolean(0.2) ? `artifact_${this.randomInt(1, 100)}` : null),
      isLatest: overrides.isLatest !== undefined ? overrides.isLatest : true,
      versionNotes:
        overrides.versionNotes || (this.randomBoolean(0.3) ? 'Updated with improvements' : null),
      ...
      dimensions:
        ...
        (artifactType === 'IMAGE' || artifactType === 'VIDEO'
          ? JSON.stringify({ width: this.randomInt(100, 2000), height: this.randomInt(100, 2000) })
          : null),
      duration:
        overrides.duration ||
        (artifactType === 'VIDEO' || artifactType === 'AUDIO' ? this.randomInt(1, 3600) : null),
      quality:
        overrides.quality ||
        (artifactType === 'IMAGE' || artifactType === 'VIDEO'
          ? this.randomElement(['low', 'medium', 'high'])
          : null),
      format: overrides.format || this.getArtifactFormat(artifactType),
      category:
        overrides.category || this.randomElement(['input', 'output', 'intermediate', 'debug']),
      tags: overrides.tags || JSON.stringify(this.generateArtifactTags(artifactType)),
      importance:
        overrides.importance || this.randomElement(['critical', 'important', 'normal', 'debug']),
      status:
        overrides.status ||
        this.randomElement<ArtifactStatus>([
          'ACTIVE',
          'ARCHIVED',
          'EXPIRED',
          'DELETED',
      ...
      lastAccessedAt:
        overrides.lastAccessedAt || (this.randomBoolean(0.6) ? this.randomDate(30) : null),
      ...
      pipelineVersion:
        overrides.pipelineVersion || `v${this.randomInt(1, 3)}.${this.randomInt(0, 9)}`,
