...
export class ExecutionFactory extends BaseFactory<PipelineRun> {
  ...
  build(overrides: Partial<ExecutionCreateInput> = {}): PipelineRun {
    ...
    const status =
      overrides.status ||
      this.randomElement<ExecutionStatus>([
        'QUEUED',
        'PENDING',
        'RUNNING',
        'COMPLETED',
    ...
    const defaultData: ExecutionCreateInput = {
      ...
      duration,
      tokensUsed:
        overrides.tokensUsed || (status === 'COMPLETED' ? this.randomInt(50, 2000) : null),
      estimatedCost: overrides.estimatedCost || this.randomInt(1, 100) / 100,
      actualCost:
        overrides.actualCost || (status === 'COMPLETED' ? this.randomInt(1, 150) / 100 : null),
      ...
      error:
        overrides.error ||
        (status === 'FAILED' ? JSON.stringify(this.generateRandomError()) : null),
      retryCount: overrides.retryCount || (status === 'FAILED' ? this.randomInt(0, 3) : 0),
      maxRetries: overrides.maxRetries || 3,
      retryStrategy:
        overrides.retryStrategy || JSON.stringify({ strategy: 'exponential', baseDelay: 1000 }),
      failureReason:
        overrides.failureReason || (status === 'FAILED' ? this.generateFailureReason() : null),
      progress: overrides.progress || this.generateProgress(status),
      currentTaskId:
        overrides.currentTaskId || (status === 'RUNNING' ? `task_${this.randomInt(1, 10)}` : null),
      ...
      version:
        overrides.version ||
        `v${this.randomInt(1, 10)}.${this.randomInt(0, 9)}.${this.randomInt(0, 9)}`,
      queuedAt,
      startedAt,
      completedAt,
  ...
  async createMany(
    count: number,
    overrides: Partial<ExecutionCreateInput> = {}
  ): Promise<PipelineRun[]> {
  ...
  async createSuccessfulExecution(
    overrides: Partial<ExecutionCreateInput> = {}
  ): Promise<PipelineRun> {
  ...
  async createLongRunningExecution(
    overrides: Partial<ExecutionCreateInput> = {}
  ): Promise<PipelineRun> {
  ...
  async createTaskExecution(
    runId: string,
    overrides: Partial<TaskExecutionCreateInput> = {}
    ...
    const taskData: TaskExecutionCreateInput = {
      ...
      nodeType:
        overrides.nodeType ||
        this.randomElement(['input', 'transform', 'ai', 'output', 'condition', 'loop']),
      status,
      inputs: overrides.inputs || JSON.stringify(this.generateRandomInputs()),
      outputs: overrides.outputs || JSON.stringify(this.generateRandomOutputs(status)),
      configuration: overrides.configuration || JSON.stringify(this.generateRandomConfiguration()),
      ...
      checkpoint:
        ...
        (this.randomBoolean(0.3) ? JSON.stringify({ step: this.randomInt(1, 10) }) : null),
      executorId: overrides.executorId || `executor_${this.randomInt(1, 5)}`,
      ...
      lastUpdateAt: completedAt || startedAt || queuedAt,
      cacheKey:
        overrides.cacheKey || (this.randomBoolean(0.4) ? `cache_${this.randomInt(1, 1000)}` : null),
  ...
  async createExecutionLog(
    runId: string,
    overrides: Partial<ExecutionLogCreateInput> = {}
  ): Promise<ExecutionLog> {
    const sequence = this.getSequence('log');
    const level =
      overrides.level || this.randomElement<LogLevel>(['DEBUG', 'INFO', 'WARN', 'ERROR']);
    ...
    const logData: ExecutionLogCreateInput = {
      ...
      category:
        overrides.category ||
        this.randomElement(['system', 'user', 'error', 'debug', 'performance']),
      source:
        overrides.source ||
        this.randomElement([
          'pipeline-engine',
      ...
      memorySnapshot:
        overrides.memorySnapshot ||
        (this.randomBoolean(0.2) ? this.randomInt(100, 1000) * 1024 * 1024 : null),
  ...
  // Performance testing
  async createPerformanceExecutions(
    pipelineId: string,
    userId: string,
    count: number
  ): Promise<PipelineRun[]> {
  ...
  private generateCompletedAt(status: ExecutionStatus, startedAt: Date | null): Date | null {
    if (!startedAt || status === 'RUNNING' || status === 'PENDING' || status === 'QUEUED')
      return null;
  ...
  private generateProgress(status: ExecutionStatus): number {
    switch (status) {
      ...
      case 'PENDING':
        return 0.1;
      case 'RUNNING':
        return this.randomInt(10, 90) / 100;
      case 'COMPLETED':
        return 1.0;
      case 'FAILED':
        return this.randomInt(5, 80) / 100;
      case 'CANCELLED':
        return this.randomInt(5, 60) / 100;
      default:
        return 0;
    }
  }
  ...
  private generateCompletedTasks(status: ExecutionStatus): number {
    ...
    switch (status) {
      ...
      case 'PENDING':
        return 0;
      case 'RUNNING':
        return this.randomInt(1, total - 1);
      case 'COMPLETED':
        return total;
      case 'FAILED':
        return this.randomInt(1, total - 1);
