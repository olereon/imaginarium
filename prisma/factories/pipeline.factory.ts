...
export class PipelineFactory extends BaseFactory<Pipeline> {
  ...
  build(overrides: Partial<PipelineCreateInput> = {}): Pipeline {
    ...
    const defaultData: PipelineCreateInput = {
      ...
      status: overrides.status || this.randomElement<PipelineStatus>(['DRAFT', 'PUBLISHED', 'ARCHIVED']),
      ...
      parentId: overrides.parentId || (this.randomBoolean(0.2) ? `pipeline_${this.randomInt(1, 100)}` : null),
  ...
  async createMany(count: number, overrides: Partial<PipelineCreateInput> = {}): Promise<Pipeline[]> {
  ...
  async createImageGenerationPipeline(overrides: Partial<PipelineCreateInput> = {}): Promise<Pipeline> {
  ...
  async createTextProcessingPipeline(overrides: Partial<PipelineCreateInput> = {}): Promise<Pipeline> {
  ...
  async createDataAnalysisPipeline(overrides: Partial<PipelineCreateInput> = {}): Promise<Pipeline> {
