 ...
 */
export async function loadFixturesByCategory(prisma: PrismaClient, category: keyof typeof fixturesByCategory) {
...
export function getFixtureStats() {
  return {
    ...
    byCategory: Object.keys(fixturesByCategory).reduce((acc, category) => {
      acc[category] = fixturesByCategory[category as keyof typeof fixturesByCategory].length
      return acc
    }, {} as Record<string, number>),
    byDifficulty: Object.keys(fixturesByDifficulty).reduce((acc, difficulty) => {
      acc[difficulty] = fixturesByDifficulty[difficulty as keyof typeof fixturesByDifficulty].length
      return acc
    }, {} as Record<string, number>),
    byCost: Object.keys(fixturesByCost).reduce((acc, cost) => {
