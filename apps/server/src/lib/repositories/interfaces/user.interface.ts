...
export interface UserActivity {
  userId: string
  type: 'login' | 'logout' | 'pipeline_created' | 'pipeline_executed' | 'file_uploaded' | 'password_changed'
...
export interface IUserRepository extends ISoftDeleteRepository<User, CreateUserInput, UpdateUserInput> {
  // Authentication methods
  findByEmail(email: string, includeDeleted?: boolean, context?: TransactionContext): Promise<User | null>
  ...
  // Password management
  resetPassword(id: string, token: string, expiresAt: Date, context?: TransactionContext): Promise<User>
  ...
  setEmailVerificationToken(id: string, token: string, expiresAt: Date, context?: TransactionContext): Promise<User>
  ...
  updateUserPreferences(userId: string, preferences: Partial<UserPreferences>, context?: TransactionContext): Promise<UserPreferences>
  ...
  // Bulk operations
  bulkUpdateRole(userIds: string[], role: UserRole, context?: TransactionContext): Promise<BulkOperationResult>
  ...
  // Usage and limits
  checkExecutionLimit(userId: string, context?: TransactionContext): Promise<{ 
    current: number
    limit: number
    canExecute: boolean
  }>
  checkPipelineLimit(userId: string, context?: TransactionContext): Promise<{ 
    current: number
    limit: number
    canCreate: boolean
  }>
