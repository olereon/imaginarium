...
export interface IBaseRepository<T, TCreate, TUpdate, TWhereInput = any> {
  ...
  findUnique(where: TWhereInput, options?: FilterOptions<T>, context?: TransactionContext): Promise<T | null>
  upsert(params: {
    where: TWhereInput
    create: TCreate
    update: TUpdate
  ...
  deleteMany(where: Partial<T> | Record<string, any>, context?: TransactionContext): Promise<BulkOperationResult>
  ...
  findWithDeleted?(options?: FilterOptions<T> & SoftDeleteOptions, context?: TransactionContext): Promise<T[]>
