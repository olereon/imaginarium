...
export class RepositoryTestHelper<T, TCreate, TUpdate> {
   ...
   */
  setupMockData(modelName: string, data: {
    create?: any
    findUnique?: any
    findFirst?: any
    findMany?: any[]
  ...
  return {
    'Repository CRUD Operations': () => {
      ...
      test('should create record', async () => {
        ...
        await helper.testCrudOperations(
          createData,
          dataFactory.updateValid(),
          expected
        )
      })
      ...
      test('should support pagination', async () => {
        const mockData = helper.createDataFactory().createArray(
          dataFactory.expectedResult,
          5
        )
