...
export class QueryTimeoutError extends RepositoryError {
  ...
  constructor(message: string, timeoutMs: number, query?: string) {
    super(message, undefined, {
...
export class CacheError extends RepositoryError {
  ...
  constructor(message: string, cause?: Error, operation?: CacheError['operation'], key?: string) {
    super(message, cause, {
...
export class RateLimitError extends RepositoryError {
  ...
  constructor(message: string, limit: number, remaining: number, resetTime: Date) {
    super(message, undefined, {
...
export class ErrorFactory {
  ...
  static fromPrismaError(error: any, operation: string = 'unknown'): RepositoryError {
    ...
    switch (error.code) {
      ...
      case 'P2025': // Record not found
        return new NotFoundError('Record not found', error.meta?.modelName, error.meta?.id);

      case 'P2024': // Timed out fetching a new connection
      case 'P2034': // Transaction failed due to a write conflict
        return new ConnectionError('Database connection error', error, undefined, true);

      case 'P2037': // Too many database connections opened
        return new ConnectionError('Too many database connections', error, undefined, false);
      ...
      default:
        ...
        if (message.includes('timeout') || message.includes('timed out')) {
          return new QueryTimeoutError('Query timeout', parseInt(error.meta?.timeout || '30000'));
        }
        ...
        return new RepositoryError(`Database error in ${operation}: ${message}`, error, {
          code: error.code,
          metadata: error.meta,
        });
    }
  ...
  static fromValidationErrors(
    ...
    return new ValidationError(`Validation failed for ${errors.length} field(s)`, { errors });
  }
