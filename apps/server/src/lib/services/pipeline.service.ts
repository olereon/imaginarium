...
export class PipelineService {
  ...
  async analyzeConfiguration(pipelineId: string): Promise<{
    ...
    const config = await this.pipelineRepository.getConfiguration(pipelineId) as PipelineConfiguration
    if (!config) {
  ...
  async optimizeConfiguration(pipelineId: string): Promise<{
    ...
    if (optimizedConfig.nodes.length < originalNodeCount) {
      optimizations.push(`Removed ${originalNodeCount - optimizedConfig.nodes.length} orphan nodes`)
    }
    ...
    if (optimizedConfig.connections.length < originalConnectionCount) {
      optimizations.push(`Removed ${originalConnectionCount - optimizedConfig.connections.length} duplicate connections`)
    }
