...
export class ExecutionService {
  ...
  async executeNextTasks(runId: string, executorId?: string): Promise<TaskExecution[]> {
    ...
    for (const task of readyTasks) {
      try {
        ...
        await this.executionLogRepo.logError(runId, task.id, error as Error, 'TASK_START_ERROR');
      }
    }
   ...
   */
  async completeTask(taskId: string, result: TaskResult): Promise<TaskExecution> {
  ...
  private getNodeDependencies(nodeId: string, configuration: PipelineConfiguration): string[] {
